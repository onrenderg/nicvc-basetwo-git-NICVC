...
namespace NICVC.ReserveNic
{
    ...
    public partial class MultipointVcNicDetailsPage : ContentPage
    {
        List<MultipointNIC> itemsource;
        ...
        public MultipointVcNicDetailsPage(string vcdate, string starttimefrmpg, string vcstatusfrmpg)
        {
            InitializeComponent();
        ...
        public List<MultipointNIC> loadtodayvcdetails(string searched_text)
        {
            ...
            {
                itemsource = multipointNICDatabase.GetMultipointNIC($"SELECT * , {buttonsvisibilitycases} from MultipointNIC where  " +
            ...
            else if (vcstatus.Contains("Ongoing"))
            {
                ...
                itemsource = multipointNICDatabase.GetMultipointNIC($"SELECT *, {buttonsvisibilitycases}  from MultipointNIC where substr(dateofvc, 7) || substr(dateofvc, 4, 2) || substr(dateofvc, 1, 2) = substr('{dateofvc}', 7) || substr('{dateofvc}', 4, 2) || substr('{dateofvc}', 1, 2) and TIME('{currenttime}') between TIME(Startingtime) and TIME(VCEndTime) and TIME(Startingtime)=TIME('{starttime}') " +
                    $" and ({searchedQueryAdd})  order by TIME(VCEndTime) ASC ").ToList();
            }
            else if (vcstatus.Contains("Upcoming"))
            {
                itemsource = multipointNICDatabase.GetMultipointNIC($"SELECT *, {buttonsvisibilitycases}  from MultipointNIC where substr(dateofvc, 7) || substr(dateofvc, 4, 2) || substr(dateofvc, 1, 2) = substr('{dateofvc}', 7) || substr('{dateofvc}', 4, 2) || substr('{dateofvc}', 1, 2) and TIME(Startingtime) >= TIME('{currenttime}') and TIME(Startingtime) =TIME('{starttime}') " +
                    $" and  ({searchedQueryAdd})  order by TIME(VCEndTime) ASC ").ToList();
            }
            else
            {
                string query = $"SELECT *, {buttonsvisibilitycases} from MultipointNIC where  substr(dateofvc, 7) || substr(dateofvc, 4, 2) || substr(dateofvc, 1, 2) = substr('{dateofvc}', 7) || substr('{dateofvc}', 4, 2) || substr('{dateofvc}', 1, 2) and Startingtime !='All' and TIME(Startingtime) =TIME('{starttime}') and ({ searchedQueryAdd}) order by TIME(VCEndTime) ASC";
                itemsource = multipointNICDatabase.GetMultipointNIC(query).ToList();
            }
            return itemsource;
        ...
        private void btn_takemethere_Clicked(object sender, EventArgs e)
        {
            ...
            Navigation.PushAsync(new TakeMeTherePage(vcid, "MultipointNic"));
        }
        ...
        private async void btn_alertme_Clicked(object sender, EventArgs e)
        {
            ...
            multipointNiclist = multipointNICDatabase.GetMultipointNIC($"Select * from MultipointNIC where VC_ID='{vcid}'").ToList();
            ...
            else
            {
                ...
                item.VC_ID = multipointNiclist.ElementAt(0).VC_ID;
                var vcdt = multipointNiclist.ElementAt(0).DateofVC;
                ...
                item.Dept_Name = multipointNiclist.ElementAt(0).Dept_Name;
                item.Important = multipointNiclist.ElementAt(0).Important;
                item.LevelName = multipointNiclist.ElementAt(0).LevelName;
                item.Org_Name = multipointNiclist.ElementAt(0).Org_Name;
                item.Purpose = multipointNiclist.ElementAt(0).Purpose;
                item.RequestedBy = multipointNiclist.ElementAt(0).RequestedBy;
                item.Startingtime = multipointNiclist.ElementAt(0).Startingtime;
                item.StudioID = multipointNiclist.ElementAt(0).StudioID;
                item.Studio_Name = multipointNiclist.ElementAt(0).Studio_Name;
                item.VCEndTime = multipointNiclist.ElementAt(0).VCEndTime;
                item.VCStatus = multipointNiclist.ElementAt(0).VCStatus;
                item.VcCoordEmail = multipointNiclist.ElementAt(0).VcCoordEmail;
                item.VcCoordIpPhone = multipointNiclist.ElementAt(0).VcCoordIpPhone;
                item.VcCoordMobile = multipointNiclist.ElementAt(0).VcCoordMobile;
                item.VcCoordName = multipointNiclist.ElementAt(0).VcCoordName;
                item.VccoordLandline = multipointNiclist.ElementAt(0).VccoordLandline;
                item.hoststudio = multipointNiclist.ElementAt(0).hoststudio;
                item.mcuip = multipointNiclist.ElementAt(0).mcuip;
                item.mcuname = multipointNiclist.ElementAt(0).mcuname;
                item.participantsExt = multipointNiclist.ElementAt(0).participantsExt;
                item.webroom = multipointNiclist.ElementAt(0).webroom;

                DateTime vcstartdate;
                try
                {
                    vcstartdate = DateTime.ParseExact(multipointNiclist.ElementAt(0).DateofVC, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                }
                catch
                {
                    vcstartdate = DateTime.ParseExact(multipointNiclist.ElementAt(0).DateofVC, "dd-MM-yyyy", CultureInfo.InvariantCulture);
