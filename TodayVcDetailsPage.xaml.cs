...
namespace NICVC
{
    ...
    public partial class TodayVcDetailsPage : ContentPage
    {
        List<TodayVc> itemsource;
        TodayVcDatabase todayVcDatabase;
        string vcstatus = App.vcstatustodayvc;
        string starttime = App.starttimetodayvc;
        ...
        public TodayVcDetailsPage()
        {
            InitializeComponent();
            todayVcDatabase = new TodayVcDatabase();
            Lbl_Header.Text = App.GetLabelByKey("nicvdconf");
            dateofvc = DateTime.Now.ToString("dd-MM-yyyy");
            lbl_user_header.Text = App.GetLabelByKey(App.vcstatustodayvc) +" - "+ DateTime.Now.ToString("dd-MM-yyyy") +" - " + App.starttimetodayvc;
        ...
        public List<TodayVc> loadtodayvcdetails(string searched_text)
        {
            ...
            {
                itemsource = todayVcDatabase.GetTodayVc($"SELECT * , {buttonsvisibilitycases} from TodayVC where VCStatus = 'Confirmed' " +
            ...
            {
                string query = $"SELECT *, { buttonsvisibilitycases} from TodayVC where VCStatus = 'Confirmed' " +
                    $" and substr(dateofvc, 7) || substr(dateofvc, 4, 2) || substr(dateofvc, 1, 2) = substr('{dateofvc}', 7) || substr('{dateofvc}', 4, 2) || substr('{dateofvc}', 1, 2) " +
                    $" and TIME(Startingtime) >= TIME('{currenttime}') and TIME(Startingtime) = TIME('{starttime}')   and ({ searchedQueryAdd}) order by TIME(VCEndTime) ASC";

                itemsource = todayVcDatabase.GetTodayVc(query).ToList();
            }
            else
            {
                string query = $"SELECT *, {buttonsvisibilitycases} from TodayVC where VCStatus = 'Confirmed' and substr(dateofvc, 7) || substr(dateofvc, 4, 2) || substr(dateofvc, 1, 2) = substr('{dateofvc}', 7) || substr('{dateofvc}', 4, 2) || substr('{dateofvc}', 1, 2) and Startingtime !='All' and TIME(Startingtime) =TIME('{starttime}') and ({ searchedQueryAdd}) order by TIME(VCEndTime) ASC";
                itemsource = todayVcDatabase.GetTodayVc(query).ToList();
            }
            return itemsource;
